...
import '../widget/text_dialog_socks.dart';
...
class _SocksState extends State<Socks> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            access.contains("view socks") || access.contains("admin")
                ? Row(
                    children: [
                      AnimatedContainer(
            ...
            SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  access.contains("view socks") || access.contains("admin")
                      ? buildDataTable()
                      : const Text("شماره دسترسی به این محتوا ندارید"),
                ],
              ),
            ),
          ],
  ...
  Future getData() async {
    var url = 'https://alirm.ir/mojoodi/getDataSocks.php';
    http.Response response = await http.get(Uri.parse(url));
    var data = jsonDecode(utf8.decode(response.body.runes.toList())) as List;
  ...
  Widget buildDataTable() {
    ...
    List<String> columns = [];
    if (checkAccess("view amount socks")) columns.add('موجودی کالا');
    if (checkAccess("view code socks")) columns.add('کد کالا');
    if (checkAccess("view name socks")) columns.add('نام کالا');
    columns.add('شماره');
  ...
  List<DataRow> getRow(List<product> products) => products.map((product prod) {
        //List cells = [prod.amount, prod.code, prod.name, prod.id];
        List cells = [];
        if (checkAccess("view amount socks")) cells.add(prod.amount);
        if (checkAccess("view code socks")) cells.add(prod.code);
        if (checkAccess("view name socks")) cells.add(prod.name);
        cells.add(prod.id);

        return DataRow(
          color: checkAccess("filter socks")
              ? MaterialStateProperty.resolveWith<Color?>(
                  (Set<MaterialState> states) {
                  // if (prod.amount.length >= 2) {
                  //   if (double.parse(prod.amount.substring(0, 2)) <= MIN)
                  //     return Colors.red[200];
                  // } else {
                  //   if (double.parse(prod.amount.substring(0, 1)) <= MIN)
                  //     return Colors.red[200];
                  // }
                  if (int.parse(prod.amount) <= MIN) return Colors.red[200];
                  return null;
                })
              : null,
          onLongPress: () {
            access.contains("delete socks") || access.contains("admin")
                ? deletedata(prod)
          ...
          cells: Utils.modelBuilder(cells, (index, cell) {
            ...
            return DataCell(
                ...
                showDialog(
                  ...
                  builder: (ctx) => AlertDialog(
                    ...
                    actions: <Widget>[
                      ...
                      TextButton(
                        onPressed: () {
                          if (access.contains("delete socks") ||
              ...
              onTap: () {
                switch (index) {
                  case 2:
                    access.contains("edit name socks") ||
                            access.contains("admin")
                        ? editName(prod)
                        : Fluttertoast.showToast(msg: "شما دسترسی ندارید");
                    break;
                  case 1:
                    checkAccess("edit code socks")
                        ? editcode(prod)
                        : Fluttertoast.showToast(msg: "شما دسترسی ندارید");
                    break;
                  case 0:
                    checkAccess("edit amount socks")
                        ? editamount(prod)
                        : Fluttertoast.showToast(msg: "شما دسترسی ندارید");
                    break;
                }
  ...
  Future editName(product editprod) async {
    ...
    if (Socks.updated) {
      ...
      Navigator.of(context)
          .push(MaterialPageRoute(builder: (context) => const Socks()));
    }
  }
    ...
    if (amount.text != "" && name.text != "" && code.text != "") {
      if (show) {
        ...
        final response = await http.post(
            Uri.parse("https://alirm.ir/mojoodi/insertdatasocks.php"),
            body: {
              "name": name.text,
              "code": code.text,
              "amount": amount.text,
  ...
  Future deletedata(product produ) async {
    final response = await http
        .post(Uri.parse("https://alirm.ir/mojoodi/deleteSocks.php"), body: {
      "id": produ.id,
    });
    final addHistory = await http.post(
        Uri.parse("https://alirm.ir/mojoodi/insertdatahistory.php"),
  ...
  void search(String query) async {
    ...
    setState(() {
      print("Clubs : ${prods.length} & clubs : ${prod.length}");
      this.query = query;
      prods = prod;
    });
  }
